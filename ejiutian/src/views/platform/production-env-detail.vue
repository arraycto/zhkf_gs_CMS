<template>
    <div class="table-type-4">
        <jt-row type="flex" align="middle">
            <h4 class="titleName">生产环境使用详情</h4>
            <div class="subTitle" style="margin-left: 10px;">合计调用量中包含已删除机器人的调用量</div>
        </jt-row>
        <jt-table :columns="typeAColumns" :dataSource="typeADataShow"  
                  rowKey='botnameid' class="mt8 jtTabel" :pagination="false">
        ...
        <jt-row style="" class="total-attr">
            ...
            <jt-col :span="5" style="margin-left:-7px">
                <span class='total-attr-totalFlow'>{{totalFlow}}</span>
            </jt-col>
        </jt-row>
        ...
        <jt-row class="mt16">
            ...
            <jt-col :span="18">
                <jt-pagination  :total="typeACount" :current='typeACurrentPage' :pageSize="typeAPageSize" showQuickJumper hideOnSinglePage style="text-align: right;"   @change="onTypeAPageChange">
                </jt-pagination>
            </jt-col>
        </jt-row>
    </div>    
    ...
    methods:{
        ...
        async perobotQuery() {
            ...
            this.perobotVuexQuery(this.userId).then((res) => {
                this.peRobotDetailList = res.result
                this.typeACount = this.peRobotDetailList.length;
        ...
        calctypeADataShow(pageIndex){
            return this.peRobotDetailList.slice((pageIndex - 1 ) * this.typeAPageSize, pageIndex * this.typeAPageSize);
        },
    ...
    computed: {
        ...
        typeAColumns(){ 
            return[
                ...
                {title: '生产环境调用量', dataIndex: 'flow', key: 'flow', className:"col-att",width:365,
...
@import "~@/assets/style/var.less";
 div.table-type-4{
    ...
    .total-attr {
        ...
        line-height: 40px;
        padding-left: 16px;
        .total-attr-totalFlow {
            margin-left: 10px
        }
        .total-attr-totalPeakGps {
            margin-left: 3px
        }
        .total-attr-totalPeakGpsNum {
            margin-left: 6px
        }
    }
    .total-bottom {
