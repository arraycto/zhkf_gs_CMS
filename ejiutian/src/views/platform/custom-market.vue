        ...
        <div id="custom-order" v-if='possibleOrder && proData.length>0'>
            <div id="order_detail" v-show="pageIdx==0">
                ...
                <div id="greyBg" style="font-size:12px">
                    <div id="addtion" style="height: 200px">
                        <p style="margin-bottom:20px; color: #333;font-size: 14px;">智能交互平台免费试用三个月</p>
                        <div id="charge-div">
                    ...
                    <div id="env" >
                        ...
                        <div class="test-env-div">
                            ...
                            <div class="env-tips tips-color">
                                ...
                                <span style="color: #337DFF;">(营销活动有效期内免费)</span>
                            </div>
                        </div>
            ...
            <div class="order-foot foot-margin" v-show="pageIdx==0">
                <div class="fixed-bot-panel" id="bottom-panel" style="position: absolute; bottom: auto; height:115px">
                    <div class="fee-wrapper">
                        ...
                        <span>{{unit}}</span>
                        <div>
                            <p class="tips-color" style="margin:0">试用须知:</p>
                            <p class="tips-color" style="margin:0">营销活动有效期为三个月,开通服务后可在三个月内免费试用</p>
                            <p class="tips-color" style="margin:0">到期后，政企用户未退订，将自动转为收费，互联网用户为续订，将冻结服务</p>
                        </div>
                    </div>
...
import { ECLOUD_PREFIX, ZNJHCONSOLE, POOLLINK, MARKET_CODE } from "@/utils/cloud.http";
...
export default {
    ...
    methods:{
                ...
                if(this.userinfo.isCustomer == 1 ) {
                    ...
                    if(!this.government) {
                        ...
                        if(userState in stateArr) {
                            ...
                            const res = await this.isPersonal()
                            if(res.body.isPersonal) {
                                this.possibleOrder = false
                                this.$router.push({path: '/market-result', query: {errorTitle: '无法参加免费试用', type: 1}})
                                this.$store.commit('hideLoading', {}, { root: true });
                                return
                            }
                        ...
                        const verifyRes =  await this.verifyGoverment()
                        if(verifyRes.body != 0) {
                            this.$router.push({path: '/market-result', query: {errorTitle: '无法参加免费试用', type: 1}}) 
                            this.$store.commit('hideLoading', {}, { root: true });
                            return
        ...
        //校验互联网用户是否企业认证
        async isPersonal() {
            const url = ECLOUD_PREFIX + 'api/web/certificate/outer/isCertificate'
            const params = {
        ...
        //校验政企用户是否有订购资格
        async verifyGoverment() {
            const url = ECLOUD_PREFIX + 'api/web/user/outer/customer/isinnerdiscount/' + this.userinfo.customerId;
            const params = {};
            const headers = {};
            return axiosGet(url, params, null, headers)
        },
            ...
            this.getProducts(productReq).then((res)=>{
                ...
                if(respBody.length>0) {
                    ...
                    for(let i=0;i<respBody.length;i++){
                        let obj = {}
                        if(respBody[i].configs.FlavorType == "标准版"){
        ...
        //校验营销编码
        bossMktVerify() {
            const url = ECLOUD_PREFIX + 'api/web/market/bossMktVerify';
            const headers = {}
            const params = {}
            if(this.isGoverment) {
                this.counter = 0
            ...
            axiosPost(url, params, data, headers).then((res)=>{
                console.log(res)
                if(res.state == 'ERROR') {
                    this.$router.push({path: '/market-result', query: {errorTitle: res.errorMessage, type: 0}}) 
                }else{
                    this.calSpend()
                    //流程查询
                    this.procedure()
